...
`reviewedCorrections` stores that evidence and three replacement whole-snapshot hashes, computed by cloning master's checkpoint entities, verifying the old class/property/value, and changing only the proven leaf.

`tickBoundaryCorrections` records a second audited fix: tick observers now run before the next tick mutates entities. Previously, checkpoint entities included the next tick's changes, and synthetic round events fired one tick early. All 60 round-event payloads are unchanged; their ticks increase by one (for example, the first start moves from 64 to 65). Final state, game-rule checkpoints, player mappings, raw events, and counts are unchanged.

These corrected hashes come from pre-seeking commit `d042991c4088ef27f01ca7ac1f31e0ec7ac0ac2c`, which already includes the signed-int32 fix. To reproduce them, add only `this.flush()` immediately after `this.enqueueEvent('tickstart', this.currentTick)` in `ParseSession.readFrame()`, then parse the exact fixture with `captureParity()`. The unmodified commit matches the original oracle plus the signed-int32 corrections; the single-line fix matches the current parser's detailed entities and round payloads. Tests overlay its nine ALL-mode checkpoint hashes and synthetic-event hashes for both entity-enabled modes, retaining all other original expectations. `tests/unit/tick-boundaries.test.ts` checks creation/deletion visibility independently of these hashes.

`serializerVectorCorrections` records the serializer-vector length fix from commit `a47dcd2c7c79bdecaa507b0ee0a35d451fbc46cc`. Comparing all nine checkpoints and final state with serializer-vector resizing disabled showed only absent vectors becoming empty arrays and stale trailing elements being truncated; retained elements matched. Disabling resizing reproduced the previous corrected golden exactly. The new hashes were captured from that commit using `captureParity()` on the exact fixture. Tests overlay only ALL-mode entity hashes; counts, game rules, players, headers, ticks, and events retain their previous expectations. `tests/unit/entityParser.test.ts` independently checks shrinking, clearing, and regrowing serializer vectors through direct, event, and WASM paths.

`observerPawnLeakCorrection` records the removal of a leaked key from commit `c337948ef9070afdf89d8db0b15fbffd2b1b2258`. The old serializer-name allowlist traversed the nested `CCSPlayer_PingServices` serializer on its own, so `CCSObserverPawn` entities kept one unprefixed key, `CCSPlayer_PingServices.m_hPlayerPing`, although their class was never traversed. The audit took the prior output at all nine checkpoints and the final state, removed only that key from observer pawns (100 removals), and confirmed the result is byte-identical to the current canonical output. `scripts/audit-observer-leak.ts` reproduces it: run it with `--strip-leak` in a checkout of the prior commit and without the flag in the current tree, then compare the texts. Tests overlay only ALL-mode entity hashes.

To reproduce the oracle, use the exact fixture and a clean checkout of the recorded master commit with `captureParity()` from the helper above, then compare against `golden.modes`. Derive correction hashes only after auditing the corresponding behavior change against the prior snapshots. Additional differences require investigation, not a broader exclusion or blind golden refresh.
