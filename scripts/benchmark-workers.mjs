...
// Usage: bun scripts/benchmark-workers.mjs DEMO [workers=2] [static|batched|dynamic] [shared|clone|transfer]
...
const [demo, countArg = '2', scheduling = 'static', storage = 'shared'] = process.argv.slice(2);
...
if (
	...
	throw new Error('Expected DEMO [workers=2, 1..16] [static|batched|dynamic] [shared|clone|transfer]');
	...
	await new Promise((resolve, reject) => {
		...
		for (let i = 0; i < workerCount; i++) {
			...
			worker.onmessage = ({ data }) => {
				...
				if (data.type === 'ready') dispatch();
				if (data.type === 'result') {
					results.push(data.result);
					console.log(
						`Interval ${data.result.id}: ${data.result.ms.toFixed(1)} ms, reconstruction ${data.result.seekMs.toFixed(1)} ms`
					);
			...
			if (storage === 'transfer') {
				...
				worker.postMessage({ type: 'init', bytes: copy, entry }, [copy.buffer]);
			} else worker.postMessage({ type: 'init', bytes: input, entry });
		}
	});
	const ms = performance.now() - started;
	results.sort((a, b) => a.id - b.id);
	for (let i = 0; i < results.length; i++) {
		const { seekMs, ms: jobMs, ...actual } = results[i];
		assert.deepEqual(actual, reference[i], `Interval ${i} differs`);
	}
	console.log(
		JSON.stringify({
			workerCount,
			...
			seekMs: results.reduce((sum, r) => sum + r.seekMs, 0),
			verified: true
		})
	);
} finally {
